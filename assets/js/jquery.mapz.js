...
(function( $ ){
  ...
  $.fn.mapz = function(options) {
		...
		map.children(".level:first").addClass('selected-level');
		...
		function createMaps(){
			...
			map.children('.level').each(function() {
				...
				if($(this).hasClass('selected-level')) return;
		...
		function zoom(direction) {
			var currentlvl = map.children(".selected-level");
			// Set direction and check if there is a deeper level to zoom to.
			switch(direction) {
				
				case 'in':
					if(map.children(".selected-level").next().length == 0) return;
					var targetlvl = currentlvl.next();
				break;
				
				case 'out':
					if(map.children(".selected-level").prev().length == 0) return;
			...
			currentlvl.removeClass('selected-level');
			targetlvl.addClass('selected-level');
